...
(function (Retool) {
    var AppGlobal = (function () {
        function AppGlobal() {
            ...
            this.FRONTEND_DOMAIN = "frontend";
            this.INDEX_TEMPLATE_NAME = "index";
            this.ADMIN_APP_NAME = "admin";
            this.assetPath = "/assets";
        }
        AppGlobal.prototype.init = function () {
            this.apps = new Retool.ApplicationLocator();
            this.canvases = new Retool.Canvases();
            var app = new Retool.Application("crm");
            this.currentApp = app;
            this.apps.add(app);
            if (!this.ide) {
                this.watchStorage();
            ...
            Retool.PageRenderer.initPage();
            this.adminBasePath = "retool";
        };
        AppGlobal.prototype.watchStorage = function () {
            var self = this;
            if (Retool.browser) {
                $(window).bind('storage', function (e) {
                    if (self.getSessionToken()) {
                        ...
                        Retool.App.username = "";
                        window.location.href = Retool.App.loginUrl;
                    }
                });
            }
        };
        AppGlobal.prototype.isModel = function (value) {
            return (value instanceof Retool.Model);
        };
        AppGlobal.prototype.isObjectModel = function (value) {
            return (value instanceof Retool.ObjectModel);
        };
        AppGlobal.prototype.isArrayModel = function (value) {
            return (value instanceof Retool.ArrayModel);
        };
        AppGlobal.prototype.isPrimitive = function (value) {
            return (value != Object(value));
        };
        AppGlobal.prototype.getLocalStorage = function (key) {
            if (!Retool.browser)
                return;
            var w = this.rootWindow || window;
            return w.localStorage.getItem(key);
        };
        AppGlobal.prototype.setLocalStorage = function (key, value) {
            if (!Retool.browser)
                return;
            var w = this.rootWindow || window;
            w.localStorage.setItem(key, value);
        };
        AppGlobal.prototype.exportVolume = function (volume) {
            var list = [];
            this.exportFolder(volume.root, list);
            return JSON.stringify(list, null, "\t");
        };
        AppGlobal.prototype.exportFolder = function (folder, list) {
            var self = this;
        ...
        AppGlobal.prototype.addError = function (e) {
            if (!this.errors) {
                this.errors = [];
            }
            this.errors.push(e);
        };
        AppGlobal.prototype.ignoreErrors = function () {
            if (this.errors) {
        ...
        AppGlobal.prototype.popErrors = function () {
        ...
        AppGlobal.prototype.clearErrors = function () {
            this.errors = null;
            this.errorIgnoreIndex = -1;
        };
        AppGlobal.prototype.makeUrl = function (domain, href) {
            return Retool.Url.domain(domain).href(href);
        };
        AppGlobal.prototype.makeWriter = function () {
            return new Retool.Writer();
        };
        AppGlobal.prototype.getLocationInfo = function (options, loc) {
            var accountId = "";
            this.apiHost = "/ghost/api/v0.1/retool/";
            this.assetRoot = options.assetRoot;
            this.setProjectInfo(accountId);
        };
        AppGlobal.prototype.setProjectInfo = function (accountId) {
            var assetDomain = accountId;
            this.accountId = accountId;
            if (!assetDomain) {
                assetDomain = "account_data/" + accountId;
            }
        };
        AppGlobal.prototype.handleVMError = function (err, component) {
            ...
            this.addError(err);
            Retool.SystemErrorRenderer.writeErrorsToPage(this.errors);
        };
        AppGlobal.prototype.showErrors = function () {
            Retool.SystemErrorRenderer.writeErrorsToPage(this.errors);
            this.clearErrors();
        };
        AppGlobal.prototype.afterLoad = function () {
            this.apps.each(function (app) {
                app.afterLoad();
            });
            if (this.ide) {
                this.ide.appLoaded(this);
            }
        };
        AppGlobal.prototype.writeConsoleMessages = function (data) {
            if (this.ide) {
        ...
        AppGlobal.prototype.getSessionToken = function () {
            var json = Retool.App.getLocalStorage('ghost:session');
            if (json) {
                try {
        ...
        AppGlobal.loginUrl = "/ghost";
        return AppGlobal;
    })();
    Retool.AppGlobal = AppGlobal;
    Retool.App = new AppGlobal();
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var Application = (function () {
        function Application(name) {
            this.name = name;
            this.volume = new Retool.FileSystem.Volume(name, { useFolders: true });
            this.templates = new Retool.TemplateLocator(this);
            this.controllers = new Retool.ControllerLocator(this);
            this.models = new Retool.ModelLocator(this);
            this.components = new Retool.ComponentLocator(this);
            this.helpers = Retool.Runtime.helpers;
        }
        Application.prototype.templateChanged = function (filename) {
            this.templates.removeFromCache(filename);
        };
...
var Retool;
(function (Retool) {
    var ApplicationLocator = (function () {
        function ApplicationLocator() {
            this._list = {};
        }
        ApplicationLocator.prototype.add = function (app) {
            this._list[app.name] = app;
        };
        ApplicationLocator.prototype.get = function (name) {
            return this._list[name];
        };
        ApplicationLocator.prototype.each = function (f) {
            for (var key in this._list) {
                var app = this._list[key];
                f(app);
            }
        };
        return ApplicationLocator;
    })();
    Retool.ApplicationLocator = ApplicationLocator;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
...
(function (Retool) {
    ...
    Retool.morph;
    Retool.browser = (typeof window != "undefined");
    Retool.eventsAttached;
    Retool.startupShiftClickElementId; // for when shift click launches IDE, need element to show after loaded
    ...
    function loadSchema(files, systemFiles) {
        ...
        var errors = [];
        var app = Retool.App.currentApp;
        if (files) {
            loadSchemaFiles(app, files, errors);
        }
        if (systemFiles) {
            loadSchemaFiles(app, systemFiles, errors);
        }
        if (errors.length) {
    ...
    function loadSchemaFiles(app, files, errors) {
        if (files) {
            ...
            ensureFolders(volume, ['models', 'api', 'admin', 'frontend', 'components']);
        }
    }
    Retool.loadSchemaFiles = loadSchemaFiles;
    function ensureFolders(volume, list) {
        for (var i = 0; i < list.length; i++) {
            var folder = volume.getFolder(list[i]);
            if (!folder) {
                volume.createFolder(list[i]);
            }
        }
    }
    ...
    function loadEnvironment(successHandler, errorHandler) {
        ...
        request.add({ className: "ProjectFiles", method: "session_status", success: function (data) {
            ...
            Retool.App.setProjectInfo(result.accountId);
            Retool.App.username = result.me.users[0].name;
            Retool.App.themeDescription = result.theme;
        }, error: function (err) {
            var e = err;
        } });
    ...
    function start(options) {
        ...
        if (!Retool.browser) {
            ...
            var canvas = new Retool.Canvas("m");
            Retool.App.mainCanvas = canvas;
            Retool.App.canvases.add(canvas);
        }
        else {
            var w = window;
            var isTopMost;
            if (w.parent.Retool && w.parent != w && w.parent.Retool.IDE && w.parent.Retool.IDE.activeDesigner) {
                // parent is Ide
                Retool.App.ide = w.parent.Retool.IDE.activeDesigner;
                Retool.App.rootWindow = w.parent;
            }
            else {
            ...
            var a = $('#retool-app');
            Retool.App.name = a.attr('retool-app');
            Retool.App.init();
            Retool.App.getLocationInfo(options, window.location);
            loadEnvironment(function success(result) {
                Retool.App.afterLoad();
                if (isTopMost) {
                    var authManager = new Retool.GhostAuthManager({
                        apiBaseURL: "/ghost/api/v0.1/",
                        onRefreshError: function (err) {
                            if (err.message == "requires_signin") {
                                alert('Siging required');
                            }
                        }
                    });
                    authManager.startAutoRefresh();
                }
                Retool.PageRenderer.renderFirstView(window.location);
            }, function (err) {
                alert(err);
            });
        }
    ...
    Retool.loadAndStartIDE = loadAndStartIDE;
    function redirectToSignIn() {
        window.location.href = Retool.AppGlobal.loginUrl;
    }
    Retool.redirectToSignIn = redirectToSignIn;
...
var Retool;
(function (Retool) {
    var GhostAuthManager = (function () {
        function GhostAuthManager(options) {
            this.defaultWait = 10000;
            //   /ghost/api/v0.1/
            options = options || {};
            this.baseURL = options.apiBaseURL;
            this.onError = options.onRefreshError;
            if (this.baseURL[this.baseURL.length - 1] !== "/") {
                this.baseURL += "/";
            }
        }
        GhostAuthManager.prototype.getAuthInfo = function () {
            var json = Retool.App.getLocalStorage('ghost:session'), auth;
            if (json) {
                try {
        ...
        GhostAuthManager.prototype.refreshToken = function (done) {
            console.log('refreshing');
            var auth = this.getAuthInfo();
            if (!auth) {
                //var error = new UnauthorizedError("Not logged in");
                var error = new Error("requires_signin");
                done(error);
                return;
            }
            ...
            $.ajax({
                ...
                Retool.App.setLocalStorage('ghost:session', JSON.stringify(auth));
                done();
                ...
                if (response.status === 401) {
                    error = new Error("requires_signin");
                    try {
        ...
        GhostAuthManager.prototype.startAutoRefresh = function () {
            var self = this;
            ...
            if (!auth) {
                if (this.onError) {
                    this.onError(new Error("requires_signin"));
                }
                self.timer = setTimeout(this.startAutoRefresh.bind(this), this.defaultWait);
            }
            else {
                var now = ((new Date()).getTime());
                var expiresAt = auth.expires_at || now;
                var wait = expiresAt - now - 5000;
                if (wait < 0)
        ...
        GhostAuthManager.prototype.stopAutoRefresh = function () {
            if (this.timer) {
                clearTimeout(this.timer);
            }
            delete this.timer;
        };
        return GhostAuthManager;
    })();
    Retool.GhostAuthManager = GhostAuthManager;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var VM;
    (function (VM) {
        function createProcess(canvas) {
            return new VM.Process(canvas);
        }
        ...
        function executeMethodSync(params) {
            ...
            var proc = new VM.Process(params.canvas);
            proc.sync = true;
    ...
    (function (VM) {
        var Process = (function (_super) {
            ...
            Process.prototype.showBreakPoint = function (callEntry, inst) {
                Retool.App.ide.showBreakPoint(this, callEntry.methodInfo.file, inst.line);
            };
            ...
            Process.prototype.getStaticTypeValue = function (name) {
                ...
                if (segments.length == 2) {
                    if (segments[0] == "api") {
                        var controllerDef = this.app.controllers.getDefinition(segments[1]);
                        if (controllerDef) {
                            return new Retool.ControllerStatic(controllerDef);
                        }
                    }
                    else if (segments[0] == "models") {
                        var modelDefinition = this.app.models.getDefinition(segments[1]);
                        if (modelDefinition) {
            ...
            Process.prototype.createNewObjForType = function (name, args) {
                ...
                var modelId = name;
                var modelDefinition = this.app.models.getDefinition(modelId);
                if (modelDefinition) {
                    var data;
                    if (args.length) {
                        data = args[0];
            ...
            Process.prototype.getNewProcess = function () {
                return VM.createProcess(this.canvas);
            };
...
(function (Retool) {
    var Html = (function () {
        ...
        Html.escape = function (string) {
            if (!string)
                return "";
            var entityMap = Html._entityMap;
            return String(string).replace(/[&<>"'\/]/g, function (s) {
                return entityMap[s];
            });
        };
        Html.escapeAttr = function (string) {
            if (!string)
                return "";
            var map = Html._attributeMap;
            return String(string).replace(/[<>\"']/g, function (s) {
                return map[s];
            });
        };
        Html.fromText = function (text) {
...
(function (Retool) {
    var SegmentedStream = (function () {
        function SegmentedStream() {
            this._segments = [];
            this._current = "";
        }
        SegmentedStream.prototype.write = function (text) {
            this._current += text;
        };
        SegmentedStream.prototype.addSegment = function (segment) {
            this.flush();
            this._segments.push(segment);
        };
        SegmentedStream.prototype.flush = function () {
            if (this._current) {
                this._segments.push(this._current);
                this._current = "";
            }
        };
        SegmentedStream.prototype.getLength = function () {
            return 0;
        };
        SegmentedStream.prototype.setLength = function (n) {
        };
        SegmentedStream.prototype.toString = function () {
            this.flush();
            return this._segments.join();
        };
        return SegmentedStream;
    })();
    Retool.SegmentedStream = SegmentedStream;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var Task = (function () {
        function Task(loop, func) {
            this.loop = loop;
            this.func = func;
        }
...
(function (Retool) {
    var TaskLoop = (function () {
        ...
        TaskLoop.prototype.add = function (func) {
            var task = new Retool.Task(this, func);
            this._tasks.push(task);
            return task;
        };
        TaskLoop.prototype.start = function (done) {
        ...
        TaskLoop.prototype.taskCompleted = function (task) {
            this._runningCount--;
            if (!this._loopRunning) {
                this.executeLoop();
            }
        };
        TaskLoop.prototype.executeLoop = function () {
            this._loopRunning = true;
            while (this._currentTask < this._tasks.length) {
    ...
    Retool.TaskLoop = TaskLoop;
    var SampleJob = (function () {
        function SampleJob() {
        }
        SampleJob.run = function () {
            var s = new Retool.SegmentedStream();
            var tasks = new TaskLoop();
            ...
            tasks.add(function (done) {
                s.write("<String 2>");
                var seg = new Retool.SegmentedStream();
                s.addSegment(seg);
                setTimeout(function () {
                    seg.write("<String 2A>");
                    s.write("<String 5>");
        ...
        SampleJob.test = function () {
            var isAsync = false;
            SampleJob.async(function () {
                console.log("isAsync=", isAsync);
            });
            isAsync = true;
        };
        SampleJob.sync = function (done) {
            done();
        };
        SampleJob.async = function (done) {
            setTimeout(function () {
                done();
            }, 0);
        };
...
(function (Retool) {
    ...
    (function (VM) {
        var Global = (function (_super) {
            ...
            Global.prototype.vm_escape = function (params, text) {
                return Retool.Html.escape(text);
            };
            return Global;
        })(VM.ObjectBase);
        VM.Global = Global;
        ...
        var VMComponent = (function (_super) {
            ...
            VMComponent.prototype.vm_nav = function (params) {
                if (this.component) {
                    ...
                    var url = Retool.Url.context(this.component.context).href(templateName).query(templateParams);
                    this.show(null);
                    if (templateName) {
                        this.nav(proc, url, options);
                    }
                }
            };
            VMComponent.prototype.nav = function (proc, url, options) {
                this.show(null);
                var c = this.component;
                var app = c.getApp();
                var template = app.templates.get(url);
                if (!template) {
                    throw new VM.VMError("Cannot find template: " + url.getFilename());
                }
                if (c && c.helper instanceof Retool.Helpers.Outlet) {
                    var outletHelper = c.helper;
                    // outletHelper.navigate(c,{url:url},function done(){});
                    return;
                }
    ...
    (function (VM) {
        var Alert = (function (_super) {
            ...
            Alert.prototype.showMsg = function (params, css) {
                ...
                w.prettyText(msg);
                w.write('</div>');
                var canvas = params.proc.canvas;
                canvas.getContentElement().prepend('<div class="container">' + w.getStream() + '</div>');
            };
            return Alert;
        })(VM.ObjectBase);
        VM.Alert = Alert;
    ...
    (function (VM) {
        var Page = (function (_super) {
            ...
            Page.prototype.getMissingProperty = function (params) {
                if (params.name[0] == "$") {
                    var canvas = params.proc.canvas;
                    var c = canvas.findComponentByName(params.name.substr(1), -1);
                    if (c) {
                        return new VM.VMComponent(c);
                    }
                }
            ...
            Page.prototype.vm_nav = function (params) {
                var proc = params.proc;
                var href = params.getIndex(0);
                var query = params.getIndex(1);
                var options = params.getIndex(2) || {};
                var redirectOptions = {};
                var canvas = proc.canvas;
                redirectOptions.url = Retool.Url.canvas(canvas).href(href).query(query);
                canvas.redirect(null, redirectOptions);
            };
            Page.prototype.vm_modal = function (params) {
                var args = params.getArgs("template", "templateParams", "options");
                var modal = Retool.App.modalCanvas;
                var canvas = params.proc.canvas;
                var app = canvas.app;
                var url = Retool.Url.canvas(canvas).href(args.template);
                var template = app.templates.get(url);
                if (!template) {
                    throw new VM.VMError("Invalid template name: " + args.template);
                }
                var async = params.proc.pause();
                var context = Retool.RenderingContext.createRoot(canvas, template, null);
                var w = new Retool.Writer();
                w.renderView(modal, context, args.templaeParams);
                context.viewModel.modalProcess = async;
            ...
            Page.prototype.vm_flash = function (params) {
                var canvas = params.proc.canvas;
                var c = canvas.findComponentByName("flash", -1);
                if (!c) {
                    return;
                }
    ...
    (function (VM) {
        var Events = (function (_super) {
            ...
            Events.prototype.vm_trigger = function (params) {
                ...
                var event = new Retool.ComponentEvent(canvas, null);
                canvas.publishAppEvent(a.name, event, a.args);
            };
            return Events;
        })(VM.ObjectBase);
        VM.Events = Events;
...
(function (Retool) {
    var ControllerStatic = (function (_super) {
        ...
        ControllerStatic.prototype.callMissingMethod = function (params) {
            ...
            ControllerStatic.serverStaticCall(proc.app, this.def.name, params.name, args, async);
        };
        ...
        ControllerStatic.serverStaticCall = function (app, controllerName, method, args, async) {
            Retool.Model.execServerCall(app, "api." + controllerName, method, null, args, async);
        };
...
(function (Retool) {
    var ControllerLocator = (function () {
        function ControllerLocator(app) {
            this.app = app;
        ...
        ControllerLocator.prototype.getFile = function (name) {
            ...
            if (filename) {
                return this.app.getFile(filename);
            }
            return null;
        };
...
(function (Retool) {
    var Model = (function (_super) {
        ...
        Model.serverInstanceCall = function (app, modelName, method, model, args, async) {
            Model.execServerCall(app, "models." + modelName, method, model, args, async);
        };
        Model.serverStaticCall = function (app, modelName, method, args, async) {
            Model.execServerCall(app, "models." + modelName, method, null, args, async);
        };
        Model.execServerCall = function (app, className, method, model, args, async) {
            var request = new Retool.Request({ type: "data" });
    ...
    var ObjectModel = (function (_super) {
        ...
        ObjectModel.prototype.getProperty = function (params) {
            ...
            if (Array.isArray(value)) {
                ...
                if (this.modelDef && this.modelDef.relationships) {
                    ...
                    if (relationship) {
                        var proc = params.proc;
                        arrayModelDef = proc.app.models.getDefinition(relationship.model);
                    }
                }
        ...
        ObjectModel.prototype.serverCall = function (methodName, params) {
            ...
            var done = Retool.makeAsync(function (err, result) {
                if (err) {
                    ...
                    if (result == self) {
                        self.notifyContainer(proc.component, "model.updated");
                    }
                    async.resume(result);
                }
            });
            Retool.Model.serverInstanceCall(proc.app, this.modelDef.name, params.name, this, args, done);
        };
        ...
        ObjectModel.prototype.vm_del = function (params) {
            ...
            var done = Retool.makeAsync(function (err, result) {
                if (err) {
                    ...
                    self.notifyContainer(proc.component, "model.deleted");
                    async.resume(result);
                }
            });
            Retool.Model.serverStaticCall(proc.app, this.modelDef.name, "destroy", [this.data.id], done);
        };
        ...
        ObjectModel.deserialize = function (info, data) {
            ...
            if (info.args) {
                ...
                var app = Retool.App.apps.get(info.args.app);
                if (app) {
                    model.modelDef = app.models.getDefinition(info.args.typeName);
                }
            }
            return model;
        };
...
(function (Retool) {
    var ModelStatic = (function (_super) {
        ...
        ModelStatic.prototype.callMissingMethod = function (params) {
            ...
            var args = params.args;
            var async = proc.pause();
            Retool.Model.serverStaticCall(proc.app, this.modelDef.name, params.name, args, async);
        };
...
(function (Retool) {
    var FieldHelper = (function () {
        ...
        FieldHelper.formatField = function (app, model, fieldName, value) {
            if (model) {
                var field = app.getField(model, fieldName);
                if (field) {
    ...
    (function (ORM) {
        var Query = (function (_super) {
            ...
            Query.prototype.execQuery = function (method, params) {
                ...
                var args = [arg];
                Retool.Model.serverStaticCall(proc.app, modelName, "query", args, async);
            };
...
(function (Retool) {
    var Component = (function () {
        ...
        Component.prototype.evaluateMethod = function (methodName) {
            return this.context.viewModel.evaluateMethod(methodName, [this.context.scopeObj]);
        };
        ...
        Component.prototype.findComponentByDefinitionId = function (id, template) {
            var file = template.file;
            return this.doFindComponentByDefinitionId(this, id, file);
        };
        Component.prototype.doFindComponentByDefinitionId = function (component, id, file) {
            var c = component;
            if (c.customComponent) {
                c = c.customComponent;
            }
            if (c.context.template.file == file && c.attr && c.attr.id == id) {
                return component;
            }
            if (c.children) {
                for (var i = 0; i < c.children.length; i++) {
                    var found = this.doFindComponentByDefinitionId(c.children[i], id, file);
                    if (found) {
                        return found;
                    }
                }
        ...
        Component.prototype.redirect = function (event, options) {
            var view = this.getView();
            if (this.attr.outlet) {
                var app = this.getApp();
                var vmComponent;
                if (this.attr.outlet == "modal") {
                    vmComponent = new Retool.VM.VMComponent(Retool.App.modalCanvas);
                }
                if (vmComponent) {
                    vmComponent.nav(null, options.url, null);
                    return;
                }
            }
            view.redirect(event, options);
        };
        Component.prototype.getWriterType = function () {
            return Retool.elemInsideDesigner("#" + this.getElementId()) ? "ds" : null;
        };
        Component.prototype.createWriter = function (options) {
            var view = this.getView();
            var writer = view.createWriter(options);
        ...
        Component.prototype.navigateToHref = function (event, href) {
            if (event) {
            ...
            if (this.attr.href == "{{history.back}}" || this.attr.href == "{{history.back()}}") {
                ...
                href = href || this.getAttr("href");
                if (href.substr(0, 5) == "http:" || href.substr(0, 6) == "https:") {
                    window.location.href = href;
                }
                else {
                    var url = Retool.Url.context(this.context).href(this.resolveHref(href));
        ...
        Component.prototype.resolveHref = function (href) {
            ...
            if (href.substr(0, 2) == "./") {
                var template = this.context.template;
                if (template) {
                    var url = Retool.Url.file(template.file);
                    if (url.templatePath) {
                        href = url.templatePath + "/" + href.substr(2);
                    }
                    else {
                        href = href.substr(2);
                    }
        ...
        Component.prototype.createLinkHref = function (href) {
            if (href.substr(0, 5) == "http:" || href.substr(0, 6) == "https:") {
                ...
                var url = Retool.Url.context(this.context).href(href);
        ...
        ErrorRenderer.render = function (options) {
            ...
            if (writeToElement) {
                ...
                if (formElement) {
                    ...
                    if (!appErrors.length) {
                        if (Retool.App.mainCanvas) {
                            Retool.App.mainCanvas.getContentElement().prepend('<div id="retool-appErrors"></div>');
                        }
                        else {
        ...
        SystemErrorRenderer.writeErrorsToPage = function (errors) {
            ...
            if (!$errorBar.length) {
                ...
                $errorBar.on('click', '[data-action]', function (event) {
                    ...
                    if (action == "element") {
                        var canvas = Retool.App.mainCanvas;
                        var url = Retool.Url.filename(filename, canvas);
                        var template = url.getTemplate(canvas.context);
                        Retool.App.ide.show({ id: id, shiftClick: false, template: template });
                    }
                    else if (action == "code") {
...
(function (Retool) {
    var PageRenderer = (function () {
        ...
        PageRenderer.initPage = function () {
        ...
        PageRenderer.renderFirstView = function (location) {
            $('#retool-login').remove();
            ...
            var w = new Retool.Writer();
            var modal = new Retool.ModalCanvas("modal");
            modal.renderModal(w, "");
            Retool.App.modalCanvas = modal;
            Retool.App.canvases.add(modal);
            w.appendToElement('body');
            var canvas = new Retool.Canvas("m");
            canvas.app = Retool.App.currentApp;
            canvas.styleElementManager = new Retool.StyleElementManager();
            canvas.modifyWindowUrl = true;
            canvas.syncIDE = true;
            Retool.App.mainCanvas = canvas;
            Retool.App.canvases.add(canvas);
        ...
        PageRenderer.attachEvents = function () {
        ...
        PageRenderer.popStateOn = function () {
            window.onpopstate = function (event) {
                if (Retool.App && Retool.App.viewRenderCount > 1) {
                    ...
                    var canvas = Retool.App.mainCanvas;
                    if (canvas.url.toDisplay() != state.main) {
                        canvas.url = Retool.Url.canvas(canvas).history(state.main);
                        canvas.draw({ isPopState: true });
                    }
                    setTimeout(function () {
        ...
        PageRenderer.clearSelection = function () {
            var sel;
...
(function (Retool) {
    var RenderingContext = (function (_super) {
        ...
        function RenderingContext(scopeName, viewModel, scopeObj, parent) {
            _super.call(this, null);
            ...
            if (scopeObj) {
                this.modelDef = scopeObj.modelDef;
            }
        }
        RenderingContext.createRoot = function (canvas, template, viewModel) {
            var root = new RenderingContext(null, viewModel, null, null);
            root.canvas = canvas;
            root.app = canvas.app;
        ...
        RenderingContext.prototype.evaluateMethod = function (methodInfo, localProperties) {
            ...
            if (this.scopeName && this.scopeObj) {
                localProperties = localProperties || {};
                localProperties[this.scopeName] = this.scopeObj;
            }
            var g = new RenderingContextGlobal(this);
            return Retool.VM.executeMethodSync({ canvas: this.canvas, target: this.viewModel, methodInfo: methodInfo, globalTarget: g, localProperties: localProperties });
        };
        RenderingContext.prototype.evaluateMethodAsync = function (methodInfo, done, localProperties) {
            ...
            var g = new RenderingContextGlobal(this);
            return Retool.VM.executeMethodAsync({ canvas: this.canvas, target: this.viewModel, methodInfo: methodInfo, globalTarget: g, localProperties: localProperties }, done);
        };
        ...
        RenderingContext.prototype.getDomain = function () {
            return this.canvas.url.domainName;
        };
    ...
    var RenderingContextGlobal = (function (_super) {
        ...
        RenderingContextGlobal.prototype.getMissingProperty = function (params) {
            ...
            if (name == "path") {
                return this.context.canvas.url.getFullTemplateName();
            }
            return null;
        };
...
(function (Retool) {
    var Writer = (function () {
        function Writer(type) {
            ...
            this.stream = new Retool.SegmentedStream();
        }
        ...
        Writer.prototype.text = function (value) {
            this.stream.write(Retool.Html.escape(value));
            return this;
        };
        Writer.prototype.writeComponentId = function (c) {
            this.write(" id=\"" + c.getElementId() + "\"");
            return this;
        };
        Writer.prototype.writeAttr = function (name, value) {
            this.write(" " + name + "=\"" + Retool.Html.escapeAttr(value) + "\"");
            return this;
        };
        ...
        Writer.prototype.startTag = function (tagName, cssclass, attributes) {
            ...
            if (!this.staticPageMode && !this.contentOnlyMode) {
                this.writeComponentId(this.component);
            }
            if (cssclass) {
                this.writeClass(cssclass);
            }
        ...
        Writer.prototype.render = function (component) {
            ...
            if (component.customComponent) {
                var ready = this.createCustomComponentViewModel(component);
                if (!ready) {
                    // async view model constructor so placeholder is rendered and replaced when async finished
                    return;
                }
            }
            this.renderComponent(component);
        };
        Writer.prototype.renderComponent = function (c) {
            var restore = this.component;
            this.component = c;
            var restoreLength = this.stream.getLength();
            var handler = c.helper;
            handler.calculateProperties(c);
            handler.init(c);
            try {
                ...
                this.stream.setLength(restoreLength);
        ...
        Writer.prototype.createCustomComponentViewModel = function (c) {
            ...
            /*
            if (App.ide && c.parent){
                viewModel = c.parent.context.viewModel.getChild(c.attr.id);
            }
            */
            Retool.ViewModel.createForContext(context, { initialValues: null }, function (err, v) {
                ready = true; // if create call ends up by synchronous ready will get set prior to async being set
                if (err) {
                    self.error(err); // do something with the error
                }
                if (async) {
                    context.viewModel = v;
                    v.rootComponent = c;
                    /*
                    if (c){
                        c.context.viewModel.children[c.attr.id] = v;
                    }
                    */
                    async.renderComponent(c);
                    return async.done(); // !! need to track error otherwise will not call done and finish
                }
                else {
                    viewModel = v;
                }
            ...
            if (ready) {
                ...
                viewModel.rootComponent = c;
                return true;
            }
            else {
                async = this.async(c, true);
                if (Retool.browser) {
                    // render placeholder
                    this.write('<div ').writeComponentId(c).write('></div>');
                }
                return false;
            }
        };
        ...
        Writer.prototype.createComponentForHandler = function (handler, viewElement, dataPrefix, context) {
            ...
            if (handler.settings.isCustom) {
                ...
                handler = app.helpers.get(contentElem.kind) || app.helpers.get("div");
                isCustom = true;
            }
            else {
            ...
            if (handler.settings.scopeContainer) {
                ...
                /*
                var componentViewModel = ViewModel.create(context.canvas,customHelper.componentDef);
                componentViewModel.rootComponent = component;
                context = new RenderingContext(null,componentViewModel,null,context);
                */
                context = new Retool.RenderingContext(null, null, null, context);
                context.type = "component";
                context.template = customHelper.componentDef;
        ...
        Writer.prototype.createChildComponent = function (viewElement, dataPrefix, context) {
            ...
            var app = this.component.getApp();
            if (viewElement.value) {
                if (modelDef) {
                    fieldDef = app.getField(modelDef, viewElement.value);
                }
            }
            if (kind == "input" || kind == "static") {
                ...
                if (kind[0] == "s") {
                    helper = app.helpers.get("static_" + inputType);
                    if (!helper || helper.settings.kind == "r-generic") {
                        helper = app.helpers.get("static_text");
                    }
                }
                else {
                    helper = app.helpers.get("input_" + inputType);
                    if (!helper || helper.settings.kind == "r-generic") {
                        helper = app.helpers.get("input_text");
                    }
                }
            }
            else {
                helper = app.helpers.get(viewElement.kind);
            }
            if (!helper) {
                helper = app.helpers.get("div");
            }
        ...
        Writer.prototype.createRootComponent = function (options) {
            ...
            if (options.component) {
                ...
                var app = options.context.app;
                var handler = app.helpers.get(options.element.kind);
        ...
        Writer.prototype.writeToComponent = function (component, done) {
            var id = component.getElementId();
            if (component.localData.morph || component.helper.settings.morph) {
                Retool.morph.setContent({ start: id, end: id + "e" }, this.getStream());
            }
            else {
                $('#' + id).html(this.getStream());
            }
            this.closeAfterUpdatePage(done);
        };
        Writer.prototype.writeToElement = function (selector, done) {
            $(selector).html(this.getStream());
            this.closeAfterUpdatePage(done);
        };
        Writer.prototype.appendToElement = function (selector, done) {
            $(selector).append(this.getStream());
            this.closeAfterUpdatePage(done);
        };
        Writer.prototype.writeToElements = function (elements, done) {
            elements.html(this.getStream());
            this.closeAfterUpdatePage(done);
        };
        Writer.prototype.replaceElement = function (selector, done) {
            $(selector).replaceWith(this.getStream());
            this.closeAfterUpdatePage(done);
        };
        Writer.prototype.closeAfterUpdatePage = function (done) {
            var self = this;
            setTimeout(function () {
                self.close(done);
            }, 1);
        ...
        Writer.prototype.writeComponentError = function (component, e) {
            this.write("<div").writeAttr("style", "display:inline-block").writeComponentId(this.component).write('><div').writeAttr("class", "component-error").write('>');
        ...
        Writer.prototype.async = function (c, isComponent) {
            var w = new Writer();
            w.component = c;
            if (!this.taskLoop) {
                this.taskLoop = new Retool.TaskLoop();
            }
            w.taskLoop = this.taskLoop;
            var task = this.taskLoop.add(null);
            if (Retool.browser) {
                if (isComponent) {
                    w.done = function () {
                        w.replaceElement('#' + c.getElementId());
                        task.done(null, null);
                    };
                }
                else {
                    w.done = function () {
                        w.writeToComponent(c.parent);
                        task.done(null, null);
                    };
                }
            }
            else {
                this.stream.addSegment(w.stream); // add the new stream as a segment;
                w.done = function () {
                    task.done(null, null);
                };
            }
        ...
        Writer.prototype.renderView = function (view, context, params) {
            var c = this.component;
            view.parent = c;
            if (c && !this.contentOnlyMode) {
                // if "light" mode skip this , do not write component id
                var childIndex = c.children.length;
                view.id = c.id + "-" + childIndex;
                c.children.push(view);
            }
            else if (!view.id) {
                view.id = "0";
            }
            ...
            var ready;
            if (viewModel) {
                ready = true; // viewModel is ready for rendering
            }
            else {
                Retool.ViewModel.createForContext(context, { initialValues: params }, function (err, v) {
                    ready = true; // if create call ends up by synchronous ready will get set prior to async being set
                    if (err) {
                        self.error(err); // do something with the error
                    }
                    if (async) {
                        ...
                        async.done(); // !! need to track error otherwise will not call done and finish
                    }
                    else {
                        viewModel = v;
                    }
            ...
            if (ready) {
                ...
                if (!view.stopped) {
                    ...
                    self.component = c;
                    if (c) {
                        c.context.viewModel.children[c.attr.id] = viewModel;
                    }
                }
            }
            else {
                async = this.async(view);
            }
        };
        Writer.prototype.close = function (done) {
            if (this.afterRenderMethods) {
                for (var i = 0; i < this.afterRenderMethods.length; i++) {
                    var func = this.afterRenderMethods[i];
                    func();
                }
            }
            if (this.taskLoop) {
                this.taskLoop.start(done);
                this.taskLoop = null;
            }
            else if (done) {
                done();
...
(function (Retool) {
    var ComponentEvent = (function (_super) {
        ...
        function ComponentEvent(component, event) {
            ...
            if (component) {
                this.canvas = component.context.canvas;
            }
        ...
        ComponentEvent.findComponent = function (element) {
            if (element) {
                var container = ComponentEvent.findCanvas(element.id);
                if (container) {
        ...
        ComponentEvent.findCanvas = function (elementId) {
            if (!elementId) {
                return null;
            }
            var segments = elementId.split('-');
            var canvasId = segments[1];
            if (canvasId.length && canvasId[0] == "i") {
                return Retool.App.ide.getCanvas(canvasId);
            }
            return Retool.App.canvases.get(canvasId);
        };
        ComponentEvent.prototype.raise = function (eventName) {
            this.pageUrl = this.canvas.url; // need this to see if a redirect happened
        ...
        ComponentEvent.prototype.next = function () {
            if (this.canvas.url != this.pageUrl) {
                // redirect happened
                return;
            }
        ...
        ComponentEvent.prototype.updateViews = function () {
            for (var i = 0; i < this.dirtyViews.length; i++) {
                ...
                var method;
                if (viewModel && viewModel.file) {
                    method = viewModel.getCompiledMethod("calculate");
                }
                if (method) {
                    var proc = Retool.VM.createProcess(view.context.canvas);
...
(function (Retool) {
    var StyleElementManager = (function () {
        ...
        StyleElementManager.prototype.updateDocument = function (app, layout) {
            this.layoutHRefMap = {};
            this.domNodes = this.getDomStyleNodes();
            this.getLayoutNodes(app, layout);
        ...
        StyleElementManager.prototype.getLayoutNodes = function (app, layout) {
            var nodes = [];
            if (layout && layout.definition && layout.definition.elements) {
                ...
                for (var i = 0; i < elements.length; i++) {
                    if (elements[i].kind == "r-stylesheet") {
                        ...
                        if (href) {
                            var node = this.createNode(app, href);
                            nodes.push(node);
        ...
        StyleElementManager.prototype.createNode = function (app, href) {
            ...
            if (folder == "css") {
                ...
                href = Retool.App.assetPath + "/" + href;
                var cssFile = app.getFile(href);
...
(function (Retool) {
    var ViewModel = (function (_super) {
        ...
        function ViewModel(canvas, template) {
            _super.call(this, null);
            this.errorVersion = 0;
            this.children = {};
            this.localData = {};
        ...
        ViewModel.createAsync = function (canvas, template, options, done) {
            var c = new ViewModel(canvas, template);
            ...
            var p = Retool.VM.createProcess(canvas);
            c.callConstructor(p, done);
        };
        ViewModel.createForContext = function (context, options, done) {
            var template = context.template;
            if (template.definition.viewModelConstructor) {
                var viewModel = new template.definition.viewModelConstructor();
                viewModel.actions = viewModel;
                return done(null, viewModel);
            }
            ViewModel.createAsync(context.canvas, context.template, options, done);
        };
        ViewModel.create = function (canvas, template) {
            var c = new ViewModel(canvas, template);
            var p = Retool.VM.createProcess(canvas);
            try {
                c.callConstructor(p);
        ...
        ViewModel.prototype.setSourceFile = function (template) {
            ...
            this.myPrototype = ViewModel.getCompiledPrototype(template.file);
        ...
        ViewModel.prototype.hasMethod = function (name) {
            this.myPrototype = ViewModel.getCompiledPrototype(this.file, { ignoreError: true });
        ...
        ViewModel.prototype.evaluateMethod = function (methodName, args) {
            ...
            var target = this;
            target.myPrototype = ViewModel.getCompiledPrototype(target.file);
            var methodInfo = target.getPrototypeMethod(methodName);
            if (methodInfo) {
                try {
                    var proc = Retool.VM.createProcess(this.canvas);
                    proc.viewModel = this;
                    proc.sync = true;
        ...
        ViewModel.prototype.getCompiledMethod = function (methodName) {
            this.myPrototype = ViewModel.getCompiledPrototype(this.file);
        ...
        ViewModel.prototype.executeEventMethod = function (options) {
            ...
            proc.onComplete = function (result) {
                ...
                if (self.rootComponent) {
                    event.addDirtyView(self.canvas);
                }
                event.resume();
            };
            event.pause();
        ...
        ViewModel.prototype.vm_raiseEvent = function (params) {
            var customComponent = (this.rootComponent) ? this.rootComponent.customComponent : null;
            if (!customComponent) {
                return;
            }
            var eventName = params.getIndex(0);
            var data = params.getIndex(1);
            var event = new Retool.ComponentEvent(customComponent, null);
            event.data = data;
...
(function (Retool) {
    var Url = (function () {
        ...
        Url.domain = function (name, app) {
            var url = new Url();
            url.domainName = name;
            url.app = app;
            return url;
        };
        Url.canvas = function (canvas) {
            var url = new Url();
            url.domainName = canvas.url.domainName;
            url.app = canvas.app;
            return url;
        };
        Url.context = function (context) {
            return Url.canvas(context.canvas);
        };
        Url.prototype.path = function (p) {
            this.templatePath = p;
            return this;
        };
        Url.prototype.name = function (name) {
            this.templateName = name;
            return this;
        };
        Url.prototype.href = function (href) {
            this.parse(href);
            return this;
        };
        Url.prototype.location = function (location) {
            this.parse(location.pathname + location.search, true);
            return this;
        };
        Url.prototype.history = function (text) {
        ...
        Url.prototype.query = function (query) {
            if (query) {
                for (var key in query) {
                    this.queryValues[key] = query[key];
                }
            }
            return this;
        };
        Url.file = function (file) {
            return Url.filename(file.getFullName());
        };
        Url.filename = function (filename, canvas) {
            var url;
            if (canvas) {
            ...
            if (filename) {
                filename = Retool.FileSystem.PathInfo.getFilenameWithoutExtension(filename);
                var segments = filename.split("/");
                url.domainName = segments[0];
                if (segments.length > 1) {
                    if (segments.length > 2) {
                        url.templatePath = segments[1];
                        url.templateName = segments[2];
                    }
                    else {
                        url.templateName = segments[1];
                    }
                }
            }
            return url;
        };
        Url.prototype.parseParameters = function (text) {
            if (!text) {
                return "";
            }
            if (text[0] == '/') {
            ...
            var parameterStart = text.indexOf('?');
            if (parameterStart > -1) {
                var parameters = text.substring(parameterStart + 1).split('&');
                for (var i = 0; i < parameters.length; i++) {
                    var parameter = parameters[i].split('=');
                    this.queryValues[parameter[0]] = decodeURIComponent(parameter[1]);
                }
                text = text.substring(0, parameterStart);
            }
            if (hashStart > -1) {
                if (parameterStart > -1) {
                    this.hashString = text.substring(hashStart + 1, parameterStart);
                }
                else {
                    this.hashString = text.substring(hashStart + 1);
                }
                text = text.substring(0, hashStart);
            }
        ...
        Url.prototype.getFilename = function () {
            var name = Retool.FileSystem.FilePath.forType(0 /* Template */) || "";
            if (name) {
            ...
            var filename = name + this.templateName + Retool.FileSystem.FilePath.DEFAULT_FILE_EXTENSION;
            return filename;
        };
        Url.prototype.getFullTemplateName = function () {
            if (this.templatePath) {
                return this.templatePath + "/" + this.templateName;
            }
            return this.templateName;
        };
        Url.prototype.parse = function (text, fromWindow) {
            text = this.parseParameters(text);
            var app = this.app;
            if (fromWindow) {
                this.domainName = Retool.App.FRONTEND_DOMAIN;
            }
            if (fromWindow && Retool.App.adminBasePath) {
                if (text == Retool.App.adminBasePath) {
                    this.domainName = "admin";
                    this.templateName = "index";
                    return;
                }
                if (text.substr(0, Retool.App.adminBasePath.length + 1) == (Retool.App.adminBasePath + "/")) {
                    this.domainName = "admin";
                    text = text.substr(Retool.App.adminBasePath.length + 1);
                }
            }
            var segments = text.split('/');
            ...
            var p = 0;
            var s;
            if (!text || p >= segments.length) {
                this.templateName = "index";
                return;
            }
            var s = segments[p++];
            var template = app.templates.get(Url.domain(this.domainName).name(s));
            if (template) {
                ...
                this.templatePath = s;
                s = segments[p];
                template = app.templates.get(Url.domain(this.domainName).path(this.templatePath).name(s));
                if (template) {
                    this.templateName = s;
                    this.template = template;
                    p++;
                    ...
                    this.templateName = "index";
                    this.template = app.templates.get(Url.domain(this.domainName).path(this.templatePath).name("index"));
                }
            }
            if (p < n) {
                this.queryValues.id = segments[p];
            }
        };
        Url.prototype.getTemplate = function (context) {
            if (this.template) {
                return this.template;
            }
            var template = context.app.templates.get(this);
            this.template = template;
            return template;
        };
        Url.prototype.toDisplay = function () {
            var display = "";
            if (this.domainName == "admin" && Retool.App.adminBasePath) {
                display += "/" + Retool.App.adminBasePath;
            }
            if (this.templatePath) {
                display += "/" + this.templatePath;
            }
            if (this.templateName != "index") {
                display += "/" + this.templateName;
            }
            if (this.queryValues.id) {
                display += "/" + this.queryValues.id;
            }
            if (this.hashString) {
                display += "#" + this.hashString;
            }
            display += "/";
            if (this.queryValues) {
                var queryString = "";
                for (var qid in this.queryValues) {
                    if (qid != "id") {
                        var queryValue = this.queryValues[qid];
        ...
        Url.prototype.getPath = function () {
            var path = this.templatePath + "/" + this.templateName;
            if (this.queryValues.id) {
                path += "/" + this.queryValues.id;
            }
            return path;
        };
...
(function (Retool) {
    var TemplateLocator = (function () {
        function TemplateLocator(app) {
            this.app = app;
            this._filePath = Retool.FileSystem.FilePath.forType(0 /* Template */);
            this._ext = Retool.FileSystem.FilePath.DEFAULT_FILE_EXTENSION;
        }
        TemplateLocator.prototype.getFile = function (url) {
            var filename = url.getFilename();
            return this.app.getFile(filename);
        };
        TemplateLocator.prototype.get = function (url) {
            var file = this.getFile(url);
            return this.getForFile(file);
        };
        TemplateLocator.prototype.getForFile = function (file) {
            if (file) {
                var template = file.compiledContent;
                if (Retool.App.ide && !template) {
                    template = this.createTemplate(file, file.getContent());
        ...
        TemplateLocator.prototype.refresh = function (template) {
            var file = this.app.getFile(template.file.getFullName());
            return this.getForFile(file);
        };
        TemplateLocator.prototype.getByFilename = function (filename) {
            var file = this.app.getFile(filename);
            return this.getForFile(file);
        };
        TemplateLocator.prototype.removeFromCache = function (filename) {
            var file = this.app.getFile(filename);
            if (file) {
                file.compiledContent = null;
            }
        };
        TemplateLocator.prototype.createTemplate = function (file, definition) {
            try {
                var t = Retool.App.ide.compileTemplate(this.app, file, Retool.App);
                return t;
            }
            catch (e) {
...
(function (Retool) {
    var ComponentLocator = (function () {
        ...
        ComponentLocator.prototype.filenameToName = function (filename) {
            ...
            return Retool.FileSystem.PathInfo.getFilenameWithoutExtension(info.filename);
        };
        ...
        ComponentLocator.prototype.get = function (name) {
            return this.getForFile(this.getFile(name));
        };
        ComponentLocator.prototype.getForFile = function (file) {
            if (file) {
        ...
        ComponentLocator.prototype.wasModified = function (filename) {
            this.removeFromCache(filename);
            var file = this.app.getFile(filename);
            this.buildCustomHelper(file);
        };
        ComponentLocator.prototype.deleted = function (filename) {
            this.removeFromCache(filename);
            var info = Retool.FileSystem.PathInfo.parseFilename(filename);
            var name = Retool.FileSystem.PathInfo.getFilenameWithoutExtension(info.filename);
            this.app.helpers.remove(name);
        };
        ComponentLocator.prototype.removeFromCache = function (filename) {
            var file = this.app.getFile(filename);
            if (file) {
                file.compiledContent = null;
            }
        ...
        ComponentLocator.prototype.createComponent = function (name, file, definition) {
            try {
                name = name.replace(/\//g, '.');
                var t = Retool.App.ide.compileComponent(this.app, name, file, Retool.App);
                return t;
            }
            catch (e) {
                Retool.App.addError(e);
        ...
        ComponentLocator.prototype.buildCustomHelpers = function () {
            var folder = this.app.volume.getFolder(this._filePath);
            var self = this;
            if (folder) {
                folder.forEachFile(function (file) {
                    self.buildCustomHelper(file);
                });
            }
        };
        ComponentLocator.prototype.buildCustomHelper = function (file) {
            var component = this.getForFile(file);
            if (component) {
                var helper = Retool.Helpers.Custom.create(component);
                var name = Retool.FileSystem.PathInfo.getFilenameWithoutExtension(file.name);
                this.app.helpers.add(name, helper);
            }
        };
...
(function (Retool) {
    ...
    (function (FileSystem) {
        (function (FileType) {
            ...
            FileType[FileType["Markdown"] = 9] = "Markdown";
            FileType[FileType["Generic"] = 10] = "Generic";
            FileType[FileType["Text"] = 11] = "Text";
    ...
    (function (FileSystem) {
        var FilePath = (function () {
            ...
            FilePath.getType = function (filename) {
                ...
                if (!isNative) {
                    var ext = FileSystem.PathInfo.getExtension(filename);
                    if (ext == "md") {
                        return 9 /* Markdown */;
                    }
                    if (ext == "txt") {
...
(function (Retool) {
    var View = (function (_super) {
        ...
        View.prototype.render = function (w) {
        };
        View.prototype.renderContent = function (w) {
            var app = this.getApp();
            this.helper = app.helpers.get("viewcontainer");
            this.children = [];
            w.component = this;
            this.renderBreadCrumbs(w);
            var template = this.context.template;
            this.renderViewElements(w, template.definition.elements);
        };
        ...
        View.prototype.createWriter = function (options) {
            return new Retool.Writer();
        };
        ...
        View.prototype.refreshRenderedComponent = function (component, parentContext, options, eachArray) {
            ...
            try {
                if (helper) {
                    ...
                    var currentModelData = (context.scopeObj) ? context.scopeObj.data : null;
                    if (helper.settings.scopeContainer) {
                        ...
                        if (currentModelData != modelData) {
                            requiresRender = true;
                            context.scopeObj = model;
                        }
                        else {
                            if (helper.settings.isLoop) {
                                nextEachArray = model;
                            }
...
(function (Retool) {
    var PageView = (function (_super) {
        ...
        PageView.prototype.refresh = function (options) {
            var viewModel = this.context.viewModel;
            var app = this.getApp();
            var template = this.context.app.templates.refresh(this.context.template);
            var context = new Retool.RenderingContext(null, viewModel, null, this.context.parent);
            context.template = template;
            var w = new Retool.Writer();
            w.component = this;
            this.render(w);
            var elem = this.getContentElement();
            w.writeToElement(elem);
        };
        PageView.prototype.afterRender = function (w) {
            if (Retool.browser) {
                Retool.App.showErrors();
            }
        };
        PageView.prototype.redirect = function (event, options) {
            this.context.canvas.redirect(event, options);
        };
...
(function (Retool) {
    var Canvas = (function (_super) {
        ...
        Canvas.prototype.redirect = function (event, options) {
            this.autoApplyDesignerAttributes();
            this.url = options.url;
            this.draw();
        };
        Canvas.prototype.refreshStylesIfNeeded = function (layout) {
            if (this.styleElementManager && this.styleElementManager.stylesNeedRefresh(layout)) {
                this.styleElementManager.updateDocument(this.app, layout);
            }
        };
        Canvas.prototype.draw = function (options) {
            var w = new Retool.Writer();
            options = options || {};
            w.staticPageMode = options.staticPageMode;
            var context = this.context;
            if (!context) {
                context = new Retool.RenderingContext(null, null, null, null);
                this.context = context;
                context.app = this.app;
            }
            this.template = this.url.getTemplate(context);
            var app = context.app;
            if (!this.template) {
                return this.render404(w, options);
            }
            var params = this.url.queryValues;
            var self = this;
            Retool.ViewModel.createAsync(this, this.template, { initialValues: params }, function (err, result) {
                if (err) {
                }
                self.viewModel = result;
                self.renderContinue(w, options);
            });
        };
        Canvas.prototype.renderContinue = function (w, options) {
            var layoutName = this.template.definition.baseLayout || "default";
            var layoutUrl = Retool.Url.canvas(this).href(layoutName);
            var context = this.context;
            var layoutTemplate;
            var reload = options.reload;
            var renderedLayoutFile = (this.layoutTemplate) ? this.layoutTemplate.file : null;
            if (!this.template.hasBodyTag) {
                layoutTemplate = layoutUrl.getTemplate(context);
            }
            this.layoutTemplate = layoutTemplate;
            var self = this;
            if (layoutTemplate) {
                if (layoutTemplate.file != renderedLayoutFile || reload) {
                    Retool.ViewModel.createAsync(this, layoutTemplate, { initialValues: null }, function (err, layoutViewModel) {
                        if (err) {
                        }
                        self.renderLayout(w, layoutTemplate, layoutViewModel, options);
                    });
                }
                else {
                    this.renderUsingCurrentLayout(w, options);
                }
            }
            else {
                this.renderTemplate(w, options);
            }
        };
        Canvas.prototype.renderUsingCurrentLayout = function (w, options) {
            this.updateWindowLocation(options);
            this.refreshRenderedComponents({});
        };
        Canvas.prototype.renderTemplate = function (w, options) {
            // render the template on the page without a layout
            this.updateWindowLocation(options);
            var app = this.app;
            var context = Retool.RenderingContext.createRoot(this, this.template, this.viewModel);
            w.renderView(this, context, null);
            w.writeToElement('#retool-app');
            this.refreshStylesIfNeeded(this.template);
        };
        Canvas.prototype.renderLayout = function (w, layoutTemplate, layoutViewModel, options) {
            this.updateWindowLocation(options);
            var app = this.app;
            var context = Retool.RenderingContext.createRoot(this, layoutTemplate, layoutViewModel);
            w.renderView(this, context, null);
            w.writeToElement('#retool-app');
            this.refreshStylesIfNeeded(layoutTemplate);
        };
        Canvas.prototype.render404 = function (w, options) {
            this.updateWindowLocation(options);
            var app = this.app;
            this.template = null;
            this.viewModel = null;
            this.layoutTemplate = null;
            var errorUrl = Retool.Url.canvas(this).name("error");
            var errorTemplate = app.templates.get(errorUrl);
            if (errorTemplate) {
                var errorViewModel = Retool.ViewModel.create(this, errorTemplate);
                var context = Retool.RenderingContext.createRoot(this, errorTemplate, errorViewModel);
                w.renderView(this, context, null);
            }
            else {
                w.write('<h1>404 error</h1>');
            }
            w.writeToElement('#retool-app');
            this.styleElementManager.updateDocument(null, null);
        };
        Canvas.prototype.updateWindowLocation = function (options) {
            if (!options.isPopState) {
                if (!this.firstRender && this.modifyWindowUrl) {
                    try {
                        var displayUrl = this.url.toDisplay();
            ...
            if (Retool.browser && Retool.App.ide && this.syncIDE) {
                Retool.App.ide.attachToViewContainer({ template: this.template, reload: options.reload });
            }
        };
        Canvas.prototype.autoApplyDesignerAttributes = function () {
        ...
        Canvas.prototype.templateChanged = function (file) {
            var context = this.context;
            var renderRequired = false;
            this.app.templateChanged(file.getFullName());
            file.version++;
            if (this.layoutTemplate && this.layoutTemplate.file == file) {
                this.layoutTemplate = this.app.templates.refresh(this.layoutTemplate);
                renderRequired = true;
            }
            else if (this.template && this.template.file == file) {
                this.template = this.app.templates.refresh(this.template);
                if (!this.layoutTemplate) {
                    renderRequired = true;
                }
            }
            if (this.layoutTemplate) {
                context.template = this.layoutTemplate;
            }
            else {
        ...
        Canvas.prototype.redraw = function () {
            ...
            var elem = this.getContentElement();
            w.writeToElement(elem);
            this.context.canvas.refreshStylesIfNeeded(this.context.template);
        };
        Canvas.prototype.componentChanged = function (file) {
            this.app.components.wasModified(file.getFullName());
            this.redraw();
        };
        Canvas.prototype.componentDeleted = function (file) {
            this.app.components.deleted(file.getFullName());
            this.redraw();
        };
        Canvas.prototype.reload = function () {
            this.draw({ reload: true });
        };
        Canvas.prototype.addError = function (error) {
        };
        return Canvas;
    })(Retool.PageView);
    Retool.Canvas = Canvas;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var Canvases = (function () {
        ...
        Canvases.prototype.add = function (canvas) {
            var id = canvas.id;
            if (!id) {
                id = (this._nextId++).toString();
                canvas.id = id;
            }
            this._list[id] = canvas;
        };
        Canvases.prototype.get = function (id) {
            return this._list[id];
        };
        return Canvases;
    })();
    Retool.Canvases = Canvases;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var ModalCanvas = (function (_super) {
        __extends(ModalCanvas, _super);
        function ModalCanvas(id) {
            _super.call(this, id);
        }
        ModalCanvas.prototype.renderModal = function (w, css) {
        ...
        ModalCanvas.prototype.renderHeader = function (w) {
        ...
        ModalCanvas.prototype.renderBody = function (w) {
            w.writeTag("div", "modal-body-content");
            w.write("</div>");
        };
        ModalCanvas.prototype.renderFooter = function (w) {
            /*
        ...
        ModalCanvas.prototype.getContentElement = function () {
            return $('#retoolModal .modal-content');
        };
        ModalCanvas.prototype.show = function (args) {
            var className = ((args && args.className) || "");
            $('#retoolModal .modal-dialog').attr('class', "modal-dialog " + className);
            $('#retoolModal').modal('show');
        };
        ModalCanvas.prototype.getViewModel = function (options) {
            var initialValues = {};
            initialValues.params = options.params || {};
            return { initialValues: initialValues };
        };
        ModalCanvas.prototype.refresh = function (options) {
            var viewModel = this.context.viewModel;
    ...
    Retool.ModalCanvas = ModalCanvas;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var ServerCanvas = (function (_super) {
        __extends(ServerCanvas, _super);
        function ServerCanvas() {
            _super.apply(this, arguments);
        }
        ServerCanvas.prototype.renderTemplate = function (w, options) {
            var app = this.app;
            var context = Retool.RenderingContext.createRoot(this, this.template, this.viewModel);
            this.writeHeadSection(w, this.template);
            w.renderView(this, context, null);
            var self = this;
            w.close(function () {
                self.done(null, w.getStream());
            });
        };
        ServerCanvas.prototype.renderLayout = function (w, layoutTemplate, layoutViewModel, options) {
            var app = this.app;
            var context = Retool.RenderingContext.createRoot(this, layoutTemplate, layoutViewModel);
            this.writeHeadSection(w, layoutTemplate);
            w.renderView(this, context, null);
            var self = this;
            w.close(function () {
                self.done(null, w.getStream());
        ...
        ServerCanvas.prototype.writeHeadSection = function (w, template) {
            w.writeTag("head", null);
            var stylesheets = this.getStylesheetElements(template);
        ...
        ServerCanvas.prototype.getStylesheetElements = function (template) {
            var list = [];
            if (template && template.definition && template.definition.elements) {
    ...
    Retool.ServerCanvas = ServerCanvas;
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    var AutoCompleteList = (function () {
        ...
        AutoCompleteList.prototype.filterLocally = function (value) {
            ...
            var context = new Retool.DropdownContext("row", Retool.ViewModel.create(this.activeComponent.context.canvas), model, null);
            items.each(function () {
                var item = $(this);
...
(function (Retool) {
    var FieldDroplistInfo = (function () {
        ...
        FieldDroplistInfo.createRenderer = function (c) {
            ...
            if (partial) {
                var app = c.getApp();
                var t = app.templates.get(partial.template);
                ...
                if (viewDef.elements && viewDef.elements.length && viewDef.elements[0].kind == "dropdown") {
                    ...
                    renderer.viewModel = Retool.ViewModel.create(c.context.canvas, t);
                    return renderer;
                }
            }
...
(function (Retool) {
    var DropdownRenderer = (function () {
        ...
        DropdownRenderer.prototype.getRowInfo = function (index) {
            ...
            var context = new Retool.DropdownContext("row", this.viewModel, new Retool.ObjectModel(row, { modelDef: null }), null);
            var id;
        ...
        DropdownRenderer.prototype.renderContainer = function (w, rows) {
            ...
            var model = new Retool.ObjectModel({}, { modelDef: null });
            var context = new Retool.DropdownContext("row", viewModel, model, null);
            context.app = this.component.context.app;
            var root = w.createRootComponent({ element: { kind: "dropdown", children: [this.rootElement] }, viewModel: viewModel, context: context });
            w.component = root;
        ...
        CodePopulatedDropdown.prototype.toDisplay = function (value) {
            if (this.dropdownElement && this.dropdownElement.text) {
                ...
                if (i != -1) {
                    ...
                    context = new Retool.RenderingContext("temp", context.viewModel, new Retool.ObjectModel(relatedObj, { modelDef: null }), null);
                }
    ...
    (function (Adapters) {
        var RetoolServerAdapter = (function () {
            ...
            RetoolServerAdapter.prototype.getHeaders = function (request) {
                var headers = { "x-retool-app": Retool.App.name };
                ...
                error: function (error) {
                    ...
                    if (error.status == 401) {
                        ...
                        if (e.message == "Please Sign In") {
                            return Retool.redirectToSignIn();
                        }
                    }
...
(function (Retool) {
    ...
    (function (Helpers) {
        var HelperLocator = (function () {
            ...
            HelperLocator.prototype.remove = function (name) {
    ...
    (function (Helpers) {
        var HelperBase = (function () {
            ...
            HelperBase.prototype.onshiftclick = function (event) {
                ...
                var compiledTemplate = (c.customComponent) ? c.customComponent.context.template : c.context.template;
                if (compiledTemplate.file) {
                    var id = (c.customComponent) ? c.customComponent.attr.id : c.attr.id;
                    if (!Retool.App.ide) {
                        event.preventDefault();
                        event.stopPropagation();
                        Retool.startupShiftClickElementId = id;
                        return Retool.loadAndStartIDE();
                    }
                    Retool.App.ide.show({ id: id, shiftClick: true, template: compiledTemplate });
                }
                if (!event.allowDefaultAction) {
                    event.preventDefault();
                }
                event.stopPropagation();
                Retool.PageRenderer.clearSelection();
            };
            ...
            HelperBase.prototype.getLabel = function (c) {
                if (c.attr.label == "(field)") {
                    ...
                    if (modelDef && c.attr.value) {
                        return c.getApp().getMultiSegmentFieldLabel(modelDef, c.attr.value).join(' ');
                    }
                }
                return c.attr.label || "";
            };
    ...
    (function (Helpers) {
        var Custom = (function (_super) {
            ...
            Custom.create = function (component) {
                ...
                helper.settings.kind = component.name;
        ...
        var Li = (function (_super) {
            ...
            Li.prototype.computeStartTagClass = function (c) {
                ...
                if (href && (c.attr.showActive == "partial" || c.attr.showActive == "exact")) {
                    ...
                    var match = this.urlMatch(c.context.canvas.url.getPath(), href, c.attr.showActive == "exact");
                    if (match) {
                        css = "active";
                    }
                }
...
(function (Retool) {
    ...
    (function (Helpers) {
        var Markdown = (function (_super) {
            ...
            function Markdown() {
                _super.apply(this, arguments);
                this.settings = { "container": false, "toolbox": true, label: "markdown", dropKind: "block", glyph: "f", tourstop: true, expandOnCreate: true, category: "html" };
                this.allowableChildren = [];
            }
            Markdown.prototype.renderStartTag = function (writer) {
                writer.startTag("div", this.computeStartTagClass(writer.component));
            };
            Markdown.prototype.renderContent = function (w) {
                var c = w.component;
                var text = c.getAttr('markdown');
                w.write(text);
            };
            Markdown.prototype.requiresRender = function (c) {
                return false;
                return c.blockChanged(); // todo
            };
            Markdown.prototype.buildEditor = function (builder) {
                builder.addMarkdown("markdown", "Markdown");
                Helpers.BlockBase.addStandardEditor(builder);
            };
            Markdown.prototype.getInspectorInfo = function (element) {
                var info = {};
                if (element.markdown) {
                    info.literal = element.markdown;
                }
                if (element.name) {
                    info.name = element.name;
                }
        ...
        Retool.Runtime.helpers.add("r-markdown", new Markdown());
    })(Helpers = Retool.Helpers || (Retool.Helpers = {}));
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    ...
    (function (Helpers) {
        var Content = (function (_super) {
            ...
            function Content() {
                _super.apply(this, arguments);
                this.settings = { "container": false, "toolbox": true, label: "content", dropKind: "block", glyph: "f", tourstop: true, expandOnCreate: true, category: "html" };
                this.allowableChildren = [];
            }
            Content.prototype.renderStartTag = function (writer) {
                writer.startTag("div", this.computeStartTagClass(writer.component));
            };
            Content.prototype.renderContent = function (w) {
                var c = w.component;
                var content = this.getContent(c);
                if (content) {
                    w.write(content);
                }
                c.localData.renderedContent = content;
            };
            Content.prototype.requiresRender = function (c) {
                var content = this.getContent(c);
                return (content != c.localData.renderedContent);
            };
            Content.prototype.getContent = function (c) {
                var filename = c.getAttr('src');
                var file = c.context.app.getFile(filename);
                if (file) {
                    if (Retool.App.ide && (!file.compiledContent || file.requiresCompile)) {
                        file.compiledContent = Retool.App.ide.compileContent(c.context.app, file, Retool.App);
                        file.requiresCompile = true;
                    }
                    // return this.context.evaluateMethod(this.attr._compiled[name],{settings:this.attr});
                    var compiled = file.compiledContent;
                    if (compiled) {
                        if (compiled.method) {
                            return c.context.evaluateMethod(compiled.method, { settings: c.attr });
                        }
                        else {
            ...
            Content.prototype.onclick = function (event, options) {
                var c = event.component;
                var attr = c.attr;
                var handled;
                var target = $(event.domEvent.target);
                if (target.is("a")) {
                    c.navigateToHref(event, target.attr("href"));
                    handled = true;
                }
                else if (attr.name) {
                    handled = handled || c.raise({ methodName: "onclick", event: event });
            ...
            Content.prototype.buildEditor = function (builder) {
        ...
        Retool.Runtime.helpers.add("r-content", new Content());
    })(Helpers = Retool.Helpers || (Retool.Helpers = {}));
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    ...
    (function (Helpers) {
        var Dropdown = (function (_super) {
            ...
            Dropdown.prototype.buildEditor = function (builder) {
                ...
                builder.addRichText("filter", "Filter");
...
(function (Retool) {
    ...
    (function (Helpers) {
        var Foreach = (function (_super) {
            ...
            function Foreach() {
                _super.apply(this, arguments);
                this.settings = { "container": true, "toolbox": true, label: "#foreach", dropKind: "helper", glyph: "]", tourstop: false, category: "helpers", isLoop: true, morph: true, expandOnCreate: true, command: true, scopeContainer: true };
                this.allowableChildren = ['*'];
            }
            Foreach.prototype.computeModel = function (c, context) {
                var attr = c.attr;
                ...
                else if (context.scopeObj) {
                    return context.scopeObj;
                }
                return null;
            };
            Foreach.prototype.renderContent = function (w) {
                var c = w.component;
                var attr = c.attr;
                if (attr.children) {
                    var rows = c.context.scopeObj;
                    if (!rows) {
            ...
            Foreach.prototype.requiresRender = function (c) {
                return c.context.getStamp() != c.localData.renderedStamp;
            };
            Foreach.prototype.buildEditor = function (builder) {
                builder.addRichText("var", "Variable", { implicitExpression: true });
                builder.addRichText("obj", "Object", { droptype: "variables", allowNonDrop: true, implicitExpression: true });
                builder.addTextProp("pageSize", "Page Size");
            };
            Foreach.prototype.getScopeContainer = function (c) {
                if (c.attr.obj) {
                    return c;
                }
                return null;
            };
            Foreach.prototype.getInspectorInfo = function (element) {
                var info = {};
        ...
        Retool.Runtime.helpers.add("r-foreach", new Foreach());
    })(Helpers = Retool.Helpers || (Retool.Helpers = {}));
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    ...
    (function (Helpers) {
        var If = (function (_super) {
            ...
            If.prototype.buildEditor = function (builder) {
                builder.addRichText("condition", "Condition", { implicitExpression: true });
            };
        ...
        var InputBase = (function (_super) {
            ...
            InputBase.prototype.onclick = function (event) {
                ...
                if (relatedRecordLink.length) {
                    var c = event.component;
                    var app = c.getApp();
                    var relationship = app.getFieldRelationship(c.getModelDef(), c.attr.value);
            ...
            InputBase.prototype.buildEditor = function (builder) {
                ...
                builder.addRichText("label", "Label");
...
(function (Retool) {
    ...
    (function (Helpers) {
        var Body = (function (_super) {
            ...
            function Body() {
                _super.apply(this, arguments);
                this.settings = { "container": false, "toolbox": true, label: "body", dropKind: "inline", glyph: "]", tourstop: false, category: "helpers", command: true, isTemplateRoot: true };
            }
            Body.prototype.renderStartTag = function (w) {
                w.startTag("div", null);
            };
            Body.prototype.renderContent = function (w) {
                var c = w.component;
                var canvas = c.context.canvas;
                var url = canvas.url;
                if (canvas.template && !canvas.template.hasBodyTag) {
                    var context = new Retool.RenderingContext(null, canvas.viewModel, null, c.context);
                    context.template = canvas.template;
                    var body = new Retool.BodyView(c.id + "-0");
                    var params = url.queryValues;
                    w.renderView(body, context, params);
                }
                c.localData.renderedUrl = url;
                c.localData.renderedVersion = canvas.template.file.version;
            };
            Body.prototype.requiresRender = function (c) {
                var canvas = c.context.canvas;
                if (canvas.url != c.localData.renderedUrl) {
                    return true;
                }
                if (Retool.App.ide) {
                    return (canvas.template.file.version != c.localData.renderedVersion);
                }
                return false;
            };
            return Body;
        ...
        Helpers.Body = Body;
        Retool.Runtime.helpers.add("r-body", new Body());
    })(Helpers = Retool.Helpers || (Retool.Helpers = {}));
})(Retool || (Retool = {}));
var Retool;
(function (Retool) {
    ...
    (function (Helpers) {
        var Outlet = (function (_super) {
            ...
            Outlet.prototype.renderContent = function (w) {
                ...
                var template;
                if (href) {
                    var url = Retool.Url.context(context).href(href);
                    var app = c.getApp();
                    template = url.getTemplate(context);
                    var depth = this.getOutletDepth(c);
                    if (template && !template.hasBodyTag && depth < Outlet.maxDepth) {
                        var viewContext = new Retool.RenderingContext(null, viewModel, null, context);
                        viewContext.template = template;
                        var view = new Retool.OutletView("");
                        var params = url.queryValues;
                        w.renderView(view, viewContext, params);
                    }
                }
                c.localData.renderedHref = href;
                c.localData.renderedTemplate = template;
                if (template) {
                    c.localData.renderedTemplateVersion = template.file.version;
                }
                else {
                    c.localData.renderedTemplateVersion = 0;
                }
            };
            Outlet.prototype.requiresRender = function (c) {
                var href = c.getAttr('href');
                if (href != c.localData.renderedHref) {
                    return true;
                }
                if (Retool.App.ide && c.localData.renderedTemplate && c.localData.renderedTemplate.file.version != c.localData.renderedTemplateVersion) {
                    return true;
                }
                return false;
            };
            Outlet.prototype.getOutletDepth = function (c) {
                var depth = 0;
                var p = c.parent;
                while (p) {
                    if (p.attr.kind == "r-outlet") {
                        depth++;
                    }
                    p = p.parent;
                }
    ...
    (function (Helpers) {
        var Th = (function (_super) {
            ...
            Th.prototype.buildEditor = function (builder) {
                builder.addRichText("text", "Text");
                builder.addRichText("value", "Value", { droptype: "variables", implicitExpression: true });
                builder.addTextProp("colspan", "Column Span");
                Helpers.BlockBase.addStandardEditor(builder);
            };
            return Th;
    ...
    (function (Helpers) {
        var RowSelector = (function (_super) {
            ...
            RowSelector.prototype.getArray = function (c) {
                if (c.localData.header) {
                    return c.context.scopeObj;
                }
                if (c.context.parent) {
                    return c.context.parent.scopeObj;
                }
                return null;
            };
    ...
    (function (Lessons) {
        var LessonManager = (function () {
            ...
            LessonManager.prototype.render = function (w) {
                var courseFrame = new CourseView("course");
                Retool.App.canvases.add(courseFrame);
                var app = courseFrame.getApp();
                var template = app.templates.get(Retool.Url.domain("lessons").href("index"));
            };
